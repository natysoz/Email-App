<template>
    <section class="header">
        ...
        <h1>{{header | to-uppercase}}</h1>
        <BookModal v-if="gSelectedBook" v-bind:book="this.gSelectedBook"/>
        <label>
            <input type="text" v-model="search" placeholder="Search a Book"/>
        </label>
        <div class="books-container">
            <BookContainer v-on:selected-book="setSelectedBook" v-bind:books="filteredBooks"/>
        </div>
    </section>
...
<script>
    ...
    export default {
        ...
        methods: {
            setSelectedBook(id) {
                this.gSelectedBook = this.books.find(book => id === book.id)
                window.scroll(0,100)
            },
        },
        props: ["header"],
        computed: {
            filteredBooks() {
                return this.books.filter(book => book.title.match(this.search));
            }
        },
        components: {
            BookContainer,
